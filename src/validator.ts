...
function validateField(field: QrisField, errors: ValidationError[]): void {
  const meta = TAGS[field.id];
  if (!meta) {
  ...
  if (Array.isArray(field.value)) {
    // Template – recurse
    field.value.forEach((child) => validateField(child, errors));
  } else {
    ...
    if (meta.maxLength !== undefined && len > meta.maxLength) {
      errors.push({
        id: field.id,
        message: `Value too long (max ${meta.maxLength})`,
      });
    }
  }
}
